 ...
 * @return
 */
std::pair<Surface_mesh, Surface_mesh> get_intersection_mesh(const Surface_mesh& mesh1, const Surface_mesh& mesh2, std::vector<std::pair<face_descriptor, face_descriptor>>& intersections) {
    // 创建两个新的 Surface_mesh 用于保存相交部分
    Surface_mesh intersection_mesh1;
    Surface_mesh intersection_mesh2;
    ...
    // 遍历交集信息
    for (const auto& intersected_pair : intersections) {
        face_descriptor face1 = intersected_pair.first;
        face_descriptor face2 = intersected_pair.second;
